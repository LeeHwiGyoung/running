...
export default function LineGraph({data , id , xKey , yKey ,reverseYAxis= false ,  width =400 ,height=200, marginBottom= 20, marginLeft=20, marginRight=20 , marginTop= 20 , setButtonText , setHoveredSegmentId, tickFormatFunc}:LineGraphProps) {
  ...
  useEffect(()=> {
    ...
    const xMax = d3.max(processedData, d => d.properties[xKey]) || 0;
    ...
    function mousemove(event : MouseEvent) {
      ...
      if (!d1) { // d1이 undefined (즉, x0가 모든 데이터보다 크거나 같을 때)
        ...
        d = mouseX - d0.properties[xKey] > d1.properties[xKey] - mouseX ? d1 : d0;
      }
      
      const tooltipX = x(d.properties[xKey]);
      ...
      focus.select('.x-hover-text')
       ...
       if(tickFormatFunc){
         setButtonText(tickFormatFunc(d.properties[yKey]))
       }else{
         setButtonText(d.properties[yKey])
       }
